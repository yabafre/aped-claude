...
- **PR creation flow opens drafts and follows a structured body shape.** Every skill that prints or runs `gh pr create` / `glab mr create` now passes `--draft` (4 sites: `aped-quick`, `aped-dev/step-08`, `aped-review/step-05`, `aped-ship`); the user marks ready (`gh pr ready <n>` / `glab mr update --ready`) once they have re-run the validation block locally. `aped-skills/writing-discipline.md` § PRs documents the substantive-PR body shape (`## Summary` → themed sections → `## Tests` → `## Validation`), keeps the compact 2–4-bullet shape for trivial PRs, and adds an explicit "no project-internal jargon" rule so a reviewer unfamiliar with the codebase can read the body without a glossary. Sentinel test `tests/pr-draft-discipline.test.js` pins both invocations and the discipline doc against regression.
...
- `aped-method disable --local` now writes a second artefact: `.aped/config.local.yaml` (gitignored), with both `aped.enabled: false` and `skill_invocation_discipline.enabled: false`. The runtime guard is unchanged in *behavior* but reads this file with **precedence over `config.yaml`** — so the team-shared config can stay `enabled: true` while the per-developer override flips the runtime check.
- `check-enabled.sh` now reads (in order): `.aped/.DISABLED` marker → `.aped/config.local.yaml` → `.aped/config.yaml`. Local override takes precedence.
- `aped-method enable` removes both the marker AND `config.local.yaml`. The `.gitignore` lines are kept (future-proof guard).
- `aped-method status` reports `disabled-local` even when only `config.local.yaml` remains (e.g. user hand-deleted the marker).
- The `.gitignore` block written by `disable --local` now lists both `.aped/.DISABLED` and `.aped/config.local.yaml`, so neither propagates to the team.

CLAUDE.md is **not** auto-regenerated by `disable --local` (would risk a commit of the modified CLAUDE.md). The `skill_invocation_discipline.enabled: false` in the local override only affects the next `--update` regeneration; if you want the "Skill Invocation Discipline" block dropped from your CLAUDE.md too, hand-edit your local CLAUDE.md or use a `CLAUDE.local.md` (Claude Code reads it as supplemental and is gitignored by default).

### Fixed — config.yaml comment no longer claims `disable` flips `aped.enabled`

The `aped:` block comment in `config.yaml` previously said "Toggle via `aped-method disable` / `aped-method enable` — those subcommands also flip the frontmatter flag and write a snapshot." False since 6.2.0 — the implementation never touched `config.yaml`. Comment rewritten to reflect reality: `aped.enabled` is the alternative manual kill switch; the marker file (`.DISABLED`) is what `disable` writes; for per-developer suppression, `config.local.yaml` (gitignored, written by `disable --local`) takes precedence.
