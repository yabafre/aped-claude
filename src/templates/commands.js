...
//
// Tier 5 (3.12.0) — every entry carries a deprecation marker. Slash commands
// remain functional on all 3.x versions; removal target is 4.0.0. Skills are
...
function renderDeprecationBanner(def, apedDir) {
  // The skill path must match the resolution used by the body line below
  // (\${CLAUDE_PROJECT_DIR}/<apedDir>/<skill>/SKILL.md). Default scaffold
  // resolves to .aped/aped-prd/SKILL.md, NOT aped/skills/aped-prd/SKILL.md.
  return `> **Deprecated since ${def.deprecatedSince}** — slash commands will be removed in ${def.removalTarget}. Invoke the skill directly: read \`${apedDir}/${def.skill}/SKILL.md\` and follow it. This shell still works on all 3.x versions.
...
export function commands(c) {
  const a = c.apedDir;
  const suppressBanner = c.config?.commands?.suppress_deprecation_banner === true;
  return COMMAND_DEFS.map((def) => {
    const banner = !suppressBanner && def.deprecated ? renderDeprecationBanner(def, a) : '';
    return {
      path: `${c.commandsDir}/${def.name}.md`,
      content: `${renderFrontmatter(def)}

${banner}Read and follow the SKILL.md at \${CLAUDE_PROJECT_DIR}/${a}/${def.skill}/SKILL.md
`,
    };
  });
}
