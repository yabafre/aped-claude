...
2. `ls {{APED_DIR}}/WORKTREE` fails. Before concluding **solo mode**, run this guard so a real worktree with a missing marker doesn't silently fall through:
   ...
   > ❌ **This is a git worktree but `{{APED_DIR}}/WORKTREE` is missing.** I refuse to fall through to solo mode — it would lose the sprint link and the Story Leader would lose track of which story/ticket/branch it owns.
   >
   > Recovery (run from this worktree, replacing the placeholders with the right values — read them from `{{OUTPUT_DIR}}/state.yaml` `sprint.stories` if you're unsure):
   >
   > ```bash
   > bash {{APED_DIR}}/scripts/write-worktree-marker.sh \
   >   --worktree     "$PWD" \
   >   --story        "<story-key>" \
   >   --ticket       "<ticket-id>" \
   ...
   > ```
   >
   > Then re-invoke `aped-story`. (If you scaffolded before 6.12.2, also `npx aped-method` to install the helper — it lives at `{{APED_DIR}}/scripts/write-worktree-marker.sh`.)
