...
> 📦 Product: `npx aped-method` — scaffolds **35 skills** + hooks into any Claude Code project. Latest stable: **v6.3.2** (2026-05-07) — adds local-only disable (`aped-method disable --local`) for per-developer suppression without committing the change. Builds on v6.3.1 (chantier U opt-in protection hotfix), v6.3.0 (artefact contracts WARN-only schemas + `--update` orphan cleanup), v6.2.0 (review slim model + `aped-purge` 35th skill + `state.yaml` schema chantier S), and v6.1.0 (sprint mode schema v3).
