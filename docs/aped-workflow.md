...
> 📦 Product: `npx aped-method` — scaffolds **35 skills** + hooks into any Claude Code project. Latest stable: **v6.3.0** (2026-05-07) — artefact contracts: WARN-only structural schemas for `story.md` / `epics.md` / `epic-context.md` (escalates to ERROR in 7.0.0), producer-side gates wired into `aped-story` / `aped-epics`, `aped-purge` reads `⚠ schema-violation`, `--update` removes engine files orphaned by template renames between releases. Builds on v6.2.0 (review slim model + `aped-purge` 35th skill + `state.yaml` schema chantier S) and v6.1.0 (sprint mode schema v3).
