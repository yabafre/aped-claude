...
> 📦 Product: `npx aped-method` — scaffolds **25 skills** (with deprecated `/aped-X` slash command aliases) + hooks into any Claude Code project. Latest stable: **v3.12.0** (2026-04-29).
> 🔗 See also: [APED — Phases](./aped-phases.md), [APED — Personas & Teams](./aped-personas.md), [APED — Team Quickstart](./aped-quickstart.md)

> ⚠️ **Slash commands deprecation (since 3.12.0)** — All 25 `/aped-X` slash commands are now deprecated. They continue to work in all 3.x versions but will be **removed in 4.0.0**. Primary invocation is the **skill description trigger** — say *"create the prd"*, *"run an architecture review"*, etc., or use the Skill tool directly. The slash commands are convenience aliases that double the maintenance surface; skill descriptions (since Tier 4 CSO principle) are now the canonical entry point.
...
3. **Skill-first invocation** (since 3.12.0) — primary invocation is the Skill tool or natural language matching `description:` triggers. Slash commands are deprecated convenience aliases (removal target 4.0.0). The CLAUDE.md template ships the **1% rule** + a 12-row rationalization table to make skill invocation reflexive.
...
- **`.claude/commands/aped-*.md`** — 25 slash commands, **deprecated since 3.12.0** (removal 4.0.0). Each shell carries a deprecation banner pointing to its skill (suppressible via `commands.suppress_deprecation_banner: true`).
...
- **Cross-tool symlinks** (auto-detected): `.opencode/skills/`, `.agents/skills/`, `.codex/skills/` → `.aped/aped-*`.

### Optional opt-in add-ons

```bash
aped-method doctor                # verify scaffold, hooks, state, commands, symlinks
...
- 📚 Full catalog: [`docs/COMMANDS.md`](https://github.com/yabafre/aped-claude/blob/main/packages/create-aped/docs/COMMANDS.md) — annotated with the deprecation banner + Status column since 3.12.0.
...
- 📦 npm: [`aped-method`](https://www.npmjs.com/package/aped-method) — latest **3.12.0** with provenance attestation.
