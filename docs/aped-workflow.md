...
> 📦 Product: `npx aped-method` — scaffolds **35 skills** + hooks into any Claude Code project. Latest stable: **v6.3.3** (2026-05-08) — `disable --local` now also writes a gitignored `config.local.yaml` override so `aped.enabled: false` is grep-able from any config dump; activation guard reads it with precedence over the team-shared `config.yaml`. Builds on v6.3.2 (local-only disable mode), v6.3.1 (chantier U opt-in protection hotfix), v6.3.0 (artefact contracts WARN-only schemas + `--update` orphan cleanup), v6.2.0 (review slim model + `aped-purge` 35th skill + `state.yaml` schema chantier S), and v6.1.0 (sprint mode schema v3).
