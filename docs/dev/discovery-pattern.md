...
**Single-letter tokens are safe.** The generator matches `{{[A-Z_]{2,}}}` so user-prose example tokens like `{{N}}`, `{{M}}`, `{{K}}` (counts in a "report this many candidates" sentence) pass through untouched. If you ever need a one-character resolver, rename it longer first.
