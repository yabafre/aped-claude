...
# → APED is enabled — 36 skills routing normally.
# or
...
**Stale snapshot recovery.** If `.aped/.DISABLED` exists but `.aped/.disable-snapshot.json` is missing (manual edits, partial backup restore), `aped-method enable` falls back to a best-effort restore — strips the flag from all 36 skills. The 15 originals lose their pre-existing opt-out, but routing is restored. Re-edit the 15 SKILL.md frontmatters by hand if you need them opt-out again, or run `aped-method --update` to re-scaffold from the package defaults.
