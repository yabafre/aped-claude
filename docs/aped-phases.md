...
### `/aped-claude`
- 📜 **Section 7 — Skill Invocation Discipline** (since 3.11.0) — the `## APED Block Template` injected into the project's CLAUDE.md now contains the **1% rule** (verbatim from Superpowers' `using-superpowers`) + 12-row rationalization table + instruction priority (user > APED skills > defaults) + skill priority order (process first, implementation second). Section 8 cheat sheet renamed `Skill cheat sheet` (slashes dropped); deprecation note added per slash-commands deprecation in 3.12.0.
...
### Slash-commands deprecation (since 3.12.0)

All 25 `/aped-X` slash commands are deprecated. They keep working in all 3.x versions; **removal target: 4.0.0**. Primary invocation is now the skill description trigger (Skill tool or natural language matching `description:`). The scaffolded `commands/aped-*.md` shells carry a deprecation banner pointing to their skill (suppressible via `commands.suppress_deprecation_banner: true` in `config.yaml`).
